...
**Trigger:** shown only when the modelled outcome actually differs under the reform. Implemented as `|totalTaxDelta| >= $1`, gated on the arithmetic and never on `dwellingType` — the same rule §3 v2.6 adopted for the degenerate sensitivity band, for the same reason: it stays correct if the winning CGT option changes. New builds keep full negative gearing and elect between the 50% discount and indexation. **Above roughly CPI growth the discount wins**, and since it *is* the old-rules calculation the delta is exactly $0 and the module hides itself with no special case (measured at 4%, 6% and 8%). **Below it, indexation wins and the module correctly shows** — a $480k VIC new build at 2% growth elects indexation for $39,051 against the discount's $47,372. The arithmetic gate is what makes both outcomes fall out for free; a `dwellingType` gate would have wrongly suppressed the second.
...
The negative arm is measured, not hypothetical: at 3% growth over 15 years the indexed post-2027 cost base outruns the gain, and the module renders the reduce arm. The delta is also **not monotonic in growth** — at 3% growth it reads +$11,089 / +$7,085 / −$15,354 across the three periods — so no "the longer you hold, the worse it gets" copy may appear anywhere in the module.

**Expanded detail — the same sale, two rulebooks.** Two-column table, nothing ranked, nothing selectable, no colour judgment. Rows: CGT at sale; the dual-era split as **tax** (not gain) on the new-rules side only; negative-gearing refunds along the way, rendered as negatives; tax on rental profits along the way; total tax. Below it, when a pool exists at sale, one note stating the pooled figure is already used up in the tax line above.

**Two deviations from v2.2, both deliberate:**
- **No separate quarantine-treatment row.** It would double-count: the quarantine difference *is* the refunds and profit-tax rows, and the pool's value at sale *is* already inside the new-rules CGT figure. Carried as the note instead — §4's reconciliation discipline applied here.
