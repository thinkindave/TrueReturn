...
**The leverage clause is conditional (v2.8).** Everything from the em dash onward — "— the difference is leverage (~M× here)" *and* the help-tip that explains leverage — renders **only when the cash return exceeds the assumed growth**. Otherwise the sentence stops after the cash figure and takes a full stop. Both renderings:

> *Leverage explains the gap:* At the **6.0%** a year growth you assumed, your cash returned **11.3%** — the difference is leverage (~**4.2×** here). `?`
>
> *It does not:* At the **1.5%** a year growth you assumed, your cash fell **2.0%**.

**Rule:** show the clause iff `cashReturnPct > assetGrowthPct` (strictly greater; equal shows the short form, because there is no difference to attribute). Decided in `calcLeverageLine` as the `leverageExplainsGap` field, not in the renderer, so it is unit-testable — there is no DOM test harness in this project.

**Rationale.** Leverage multiplies the growth that lands on the cash stake, so it can only ever push the cash return *further from zero in the same direction* as growth. When the cash return instead sits below the growth rate, the gap is holding costs and tax, not leverage: on the default property (650k, 20% deposit, QLD) 1.5% growth yields −2.0% on cash, and no leverage multiple applied to +1.5% produces −2%. Asserting leverage there states a false cause, which is exactly the kind of comfortable misreading this product exists to prevent. Note the units when implementing: `expectedGrowth` is a fraction, the cash return is already a percentage. The help-tip must hide with the clause — a tip explaining leverage sitting beside a sentence that no longer mentions leverage is worse than no tip. The verb branches on the sign of the cash return only; the asset figure carries its own sign. The cash figure is rendered as an absolute value so a negative return reads "your cash fell 21.6%", never "fell −21.6%".
...
**Deferred, not dropped (v2.7).** The benchmark is the next slice; it consumes the leverage line's outputs (the property's return on cash supplies the X.X%, the leverage multiple supplies M×). Nothing below is implemented yet.
